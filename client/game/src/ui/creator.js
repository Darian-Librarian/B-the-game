...
const handleToggle = () => {
  ...
  btnMain.innerText = isSignUpMode ? 'Create Account' : 'Login';
...
btnMain.addEventListener('click', async () => {
  console.log("Auth Button Clicked. Mode:", isSignUpMode ? "Sign Up" : "Login");
  ...
  try {
    if (isSignUpMode) {
      ...
      if (result.success) {
        ...
        initSelection(result.account);
      } else {
        showModal("Auth Failure", "Invalid login information.");
      }
    }
  } catch (err) {
    console.error("Auth Error:", err);
    showModal("System Error", err.message || "An Unexpected Error Occurred. Check The Console.");
  }
});
...
if (btnPlay) {
  btnPlay.addEventListener('click', () => {
    ...
    if (!activeSlot) {
      return showModal("Selection Error", "Please select a character to play.");
    }
    ...
    const selectedChar = currentAccount.characters.find(c => c.name.toLowerCase() === charName.toLowerCase());
    
    if (!selectedChar) {
      return showModal("Data Error", "Could not load character data.");
    }
    ...
    import(`./game/engine.js?v=${Date.now()}`).then(module => {
      ...
      window.currentGameEngine = new module.GameEngine('game-canvas', selectedChar, currentAccount.uuid);
    }).catch(err => {
      console.error("Engine Import Error:", err);
      showModal("Engine Error", "Failed to load engine.js. Open your browser console (F12) to see the exact file path error.");
    });
  });
}
...
const heritageData = {
  ...
  primal: {
    ...
    affinities: [
      ...
      { id: 'fairy', name: 'Fairy', desc: 'Compact frame with high energy recovery and mystical assist compatibility.', spriteDir: 'primal', check: v => v >= -70 && v <= 70 },
  ...
  data.affinities.forEach((affinity, index) => {
    ...
    item.addEventListener('click', () => {
      if (item.classList.contains('locked')) {
        return showModal("Integrity Restriction", "Your current Integrity level cannot support this Affinity.");
      }
...
classItems.forEach(item => {
  item.addEventListener('click', () => {
    if (item.classList.contains('locked')) {
      return showModal("Feature Locked", "This Heritage Class Is Currently Unavailable.");
    }
...
genderItems.forEach(item => {
  item.addEventListener('click', () => {
    if (item.classList.contains('locked')) {
      return showModal("Feature Locked", "This Gender Option Is Currently Unavailable.");
    }
...
const renderStyle = (styleKey) => {
  ...
  data.skills.forEach(skill => {
    const item = document.createElement('div');
    item.className = 'list-item';
    item.style.cursor = 'default'; // Skills are not clickable
    item.innerHTML = `
      <h4>${skill.name}</h4>
      <p>${skill.desc}</p>
    `;
...
if (btnSaveChar) {
  ...
  btnSaveChar.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!result.available) return showModal("Name Unavailable", `The name "${charName}" is already taken.`);
    } catch (error) {
  ...
  document.getElementById('confirm-yes').onclick = async () => {
    ...
    try {
      const updatedAccount = await auth.createCharacter(currentAccount.uuid, pendingCharData);
      currentAccount = updatedAccount;
      
      const newChar = currentAccount.characters.find(c => c.name === pendingCharData.name);
...
const loadPowersets = async () => {
  ...
  renderArchetypes(); // Render archetypes only once JSON data is fully loaded!
};
...
const getAvailablePowersets = (label) => {
  ...
  if (!match) {
    ...
    const types = match[1].split('/'); // e.g. ["Ranged", "Melee"]
    types.forEach(t => {
  ...
  arch.cols.forEach((col, colIndex) => {
    ...
    const renderPowers = (set, preservedPicks = []) => {
      ...
      set.powers.forEach((p, i) => {
        ...
        item.addEventListener('click', () => {
          if (item.classList.contains('locked')) {
            showModal("Power Locked", "You must select earlier powers in this tree before unlocking this one.");
            return;
          }
          ...
          if (!isActive) {
            if (currentPicks <= 0) {
              return showModal("Max Powers Reached", "You have no more power picks remaining for this column.");
            }
            item.classList.add('active');
            currentPicks--;
          } else {
      ...
      // Run unlock check once on load if we preserved picks
...
nameCheckIcon.addEventListener('click', async () => {
  ...
  if (!charName) {
    return showModal("Input Error", "Character name cannot be empty.");
  }
  ...
  try {
    ...
    if (result.available) {
      showModal("Name Available", `The name "${charName}" is available!`);
    } else {
      showModal("Name Unavailable", `The name "${charName}" is already taken.`);
    }
  } catch (error) {
    showModal("Error", "Could not check name availability. Please try again later.");
...
if (btnGameMenu && gameDropdown) {
  ...
  document.getElementById('btn-save-ig-id').addEventListener('click', async () => {
    ...
    try {
      const updatedAccount = await auth.updateCharacter(currentAccount.uuid, char);
      currentAccount = updatedAccount;
      document.getElementById('in-game-id-modal').style.display = 'none';
      showModal("Success", "Citizen Identification Updated!");
    } catch (err) {
      showModal("Update Failed", err.message);
    }
  });
